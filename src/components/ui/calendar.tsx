...
function Calendar({
	...
	const buttonNavClassName = buttonVariants({
		variant: 'outline',
		className: 'absolute h-7 w-7 bg-transparent p-0 opacity-50 hover:opacity-100 cursor-pointer',
	});
	...
	return (
		<div className={cn('grid grid-cols-4 gap-y-2', className)} {...props}>
			{Array.from({ length: displayYears.to - displayYears.from + 1 }, (_, i) => {
				const currentYearDateStart = new Date(displayYears.from + i, 0, 1);
				const currentYearDateEnd = new Date(displayYears.from + i, 11, 31);

				const isBefore = startMonth
					? differenceInCalendarDays(currentYearDateEnd, startMonth) < 0
					: false;

				const isAfter = endMonth
					? differenceInCalendarDays(currentYearDateStart, endMonth) > 0
					: false;
